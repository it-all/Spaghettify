...
abstract class Controller
{
    ...
    protected function setIndexFilter(Request $request, Response $response, $args, array $listViewColumns, string $redirectRoute, ListView $view)
    {
        ...
        if (!isset($_SESSION[SESSION_REQUEST_INPUT_KEY][$view->getSessionFilterFieldKey()])) {
        ...
        if (!$filterColumnsInfo = $this->getFilterColumns($view->getSessionFilterFieldKey(), $listViewColumns)) {
            ...
            $_SESSION[$view->getSessionFilterValueKey()] = $_SESSION[SESSION_REQUEST_INPUT_KEY][$view->getSessionFilterFieldKey()];
    ...
    {
        $filterColumnsInfo = [];
        $filterParts = explode(",", $_SESSION[SESSION_REQUEST_INPUT_KEY][$filterFieldName]);
